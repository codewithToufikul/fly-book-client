  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axiosPublic.delete(`/delete-product/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="p-2 sm:p-3 lg:p-4">
                  <h2 className="text-sm sm:text-base lg:text-lg font-semibold mb-2 line-clamp-2 leading-tight" title={product.title}>
                    {product.title}
                  </h2>
                  ...
                  <div className="space-y-1 mb-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between">
                        <p className={`text-xs ${product.stock > 10 ? 'text-green-600' : product.stock > 0 ? 'text-yellow-600' : 'text-red-600'}`}>
                  ...
                  <div className="flex gap-1 sm:gap-2">
                    <button
                      ...
                      className="flex-1 bg-teal-700 text-white py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg hover:bg-teal-800 transition-colors font-medium text-xs sm:text-sm"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(product._id)}
                      className="flex-1 bg-red-600 text-white py-1.5 sm:py-2 px-2 sm:px-3 rounded-lg hover:bg-red-700 transition-colors font-medium text-xs sm:text-sm"
                    >
                      Delete
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden relative">
              ...
              <div className="sticky top-0 bg-white border-b px-4 sm:px-6 py-3 sm:py-4 rounded-t-xl sm:rounded-t-2xl z-10">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-teal-700">Edit Product</h2>
                  <button
              ...
              <div className="overflow-y-auto max-h-[calc(95vh-60px)] sm:max-h-[calc(90vh-80px)]">
                <form onSubmit={handleEdit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                  ...
                  {formData.originalPrice && formData.price && (
                    <div className="bg-teal-50 border border-teal-200 rounded-lg p-3 sm:p-4">
                      <h3 className="font-semibold text-teal-800 mb-2 text-sm sm:text-base">Price Summary</h3>
                      <div className="grid grid-cols-2 gap-2 sm:gap-4 text-xs sm:text-sm">
                        <div>
                          <span className="text-gray-600">Original Price:</span>
                          <span className="ml-1 sm:ml-2 font-semibold">${formData.originalPrice}</span>
                        </div>
                        <div>
                          <span className="text-gray-600">Current Price:</span>
                          <span className="ml-1 sm:ml-2 font-semibold text-teal-700">${formData.price}</span>
                        </div>
                        <div>
                          <span className="text-gray-600">You Save:</span>
                          <span className="ml-1 sm:ml-2 font-semibold text-green-600">
                            ${(parseFloat(formData.originalPrice || 0) - parseFloat(formData.price || 0)).toFixed(2)}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-600">Discount:</span>
                          <span className="ml-1 sm:ml-2 font-semibold text-red-600">{formData.discount || 0}%</span>
                        </div>
                      </div>
                    </div>
                  )}
